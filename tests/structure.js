#!/usr/bin/env node
// tests/structure.js: the registration lists must match the files on disk (D-003, D-010).
//
//   node tests/structure.js
//
// No browser, no server, no dependencies. Exit code 0 = everything registered.
// Guards the two places where a new file fails silently if it is not listed:
//   - resources/core/manifest.json (what bridge.js copies into Pyodide; generated by tools/gen_manifest.py)
//   - resources/scripts/asset_manifest.js (what index.html loads)
...
// 1. resources/core/manifest.json vs the three directories.
const manifestPath = path.join(CORE, 'manifest.json');
let manifest = null;
try { manifest = JSON.parse(fs.readFileSync(manifestPath, 'utf8')); } catch (e) { /* reported below */ }
...
// 2. bridge.js must not grow its hand-typed lists back.
...
// 3. Every file asset_manifest.js loads exists, and every script under resources/scripts/ is loaded.
